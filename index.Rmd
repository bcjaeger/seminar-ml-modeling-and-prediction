---
title: "Machine Learning for Prediction and Modeling"
...
# Prediction accuracy in new data $\approx$ bias $+$ variance
...
stepf = paste( 
  'Based on internally validated predictiona accuracy estimates, pick one',
  'algorithm to develop a prediction model using the full derivation data.',
  sep=' \n')
...
What you predict depends on the outcome:

- Continuous outcome $\Rightarrow$ continuous prediction

- Categorical outcome $\Rightarrow$ probabilistic prediction

- Survival outcome $\Rightarrow$ continuous OR probabilistic prediction

---

There is a common thread for prediction evaluation: 

$$\frac{1}{n}\sum_{i = 1}^n \left(\text{Prediction for person i} - \text{Observed value for person i}\right)^2 $$
- For continuous outcomes, its called mean square error

- For categorical outcomes, its called the Brier score (sometimes Graf score)

- For survival outcomes, its called the integrated Brier score 

---

### Mean squared error, scaling, and R-squared

Mean squared error needs context to be informative.

- Is it better than the error of something that already exists?

- Is it better than the error of something that is simple?

A nice way to improve the interpretability of squared error is to scale it:

$$1 - \frac{\text{your model's squared error}}{\text{reference squared error}}$$

This is known as an $R^2$ statistic for prediction because it is usually between 0 and 1. 

(It can be less than one if your model does worse than the reference.)

---

### Brier Score for binary outcomes

The **Brier score** of a predicted probability $\widehat{f} \in (0, 1)$ for a binary outcome $Y \in \left\{ 0, 1 \right\}$ is $$\frac{1}{n}\sum_{i=1}^n\left( \widehat{f}_i -  Y_i \right)^{2}.$$

**Examples** 

- If $\widehat{f}_i = 0.10$ and $Y_i=0$, the contribution to the **Brier score** is $(0.10 - 0.00)^{2} = 0.01$.

- If you predict a probability of 1/2 for everyone, the **Brier score** is always 0.25. Why?

The **Brier score** can be scaled just like mean squared error:

$$1 - \frac{\text{your model's Brier score}}{\text{reference Brier score}}$$

---

### Brier Score for survival outcomes

For survival outcomes, there is a $t$ where we evaluate our model's prediction. 

**Example**: 10-year risk for cardiovascular disease (i.e., $t = \text{10 years}$)

For predicted probability $\widehat{f}(t) \in (0, 1]$ and observed event $Y(t) \in \left\{ 0, 1 \right\}$: $$\text{Brier score} = \frac{1}{n}\sum_{i=1}^n\left( \widehat{f}(t) -  Y(t) \right)^{2}.$$

But how do we know which value of $t$ we should focus on?
...
Then sum and average over $n$ observations: 
...
You can scale the Brier score just like mean squared error.

**Example** Suppose cumulative incidence of an outcome is 10% at $t$ years, so 10% would be a good reference prediction for $t$ years. Thus, the reference Brier score is:

$$(0.1) \cdot (1–0.1)^{2} + (1–0.1) \cdot (0.1)^{2} = 0.090$$ 

So if our model gets a Brier score of 0.05, we could scale using 

$$1 - \frac{\text{our Brier score}}{\text{reference Brier score}} = 1 - \frac{0.05}{0.090} = 0.444$$

Note that this is sometimes called the **index of prediction accuracy (IPA)** and it works just the same for integrated Brier scores
...
- To estimate how a model's predictions change with respect to $X_{\text{interest}}$, we compute
...
mdl_tree_nonrandom <- ranger::ranger(
 formula = species ~ flipper_length_mm + bill_length_mm, 
 data = penguins,
 probability = TRUE,
 num.trees = 1, 
...
plots <- map(
  ...
  ~ ggplot(.x) +
    ...
    geom_point(size = 0.5, alpha = 0.2, shape=0, aes(color = name)) +
...
Contours of predictions from a non-random, **individual tree**
...
Contours of predictions from individual tree **in a random forest**
...
Boosting has dependency in its ensemble by design.

Basic idea:

1. Start with a simple prediction (learner 1)

1. Measure your error (residuals)

1. Fit a weak learner to the previous learner's error (learner i)

1. Add the current learner's prediction to your current prediction

1. if i = max boosting steps, **stop**. Otherwise go back to step 2

That's it! We can code this loop in 5 commands.

```{r, echo=TRUE}

# setting up for boosting example
library(party)
library(splines)
library(table.glue)
library(tidyverse)
...
First simulate a dataset to model

```{r, echo=TRUE}

# set the number of observations to use for the example data
nobs <- 1000
# create x, the predictor variable
x <- seq(0,1, length.out=nobs)
# create y, which has a non-linear relationship (sin function) with x
y <- sin(2*pi*x) + rnorm(nobs, mean = 0, sd = 1/5)
...
Next initialize your learning parameters

```{r, echo=TRUE}

# Initialize predictions as the mean value of y
preds <- mean(y)

# set decision trees to be 'stumps', i.e., a max depth of 1
ctrl = ctree_control(maxdepth = 1, mincriterion = 0)

# set learning rate
eta = 1/5

# initialize the list of trees, error values, and data for plotting
trees <- rmse <- ggdata <- vector(mode='list', length = 100)
...
for(t in seq_along(trees)){
  ...
  df = tibble(x = x, y = y, residuals = y - preds)
  
  trees[[t]] <- party::ctree(residuals ~ x, 
                             data = df, 
                             controls = ctrl)
  
  preds <- preds + 
    as.numeric(predict(trees[[t]], newdata = df)) * eta
  
  ggdata[[t]] <- tibble(x=x, y=y, preds=preds)
  
  rmse[[t]] <- sqrt(mean((preds-y)^2))
...
What this code looks like as it runs

```{r}
# Creates animation showing a boosting model's prediction
...
gg <- ggdata %>%
  ...
  geom_line(aes(x = x, y = preds), size = 2, col = 'red') +
  theme_bw() +
  theme(panel.grid = element_blank(),
        text = element_text(size = 18)) +
  labs(x = 'Predictor variable',
       y = 'Outcome variable',
       title = 'Predictions from ensemble of {frame_time} trees') +
...
You don't have to use trees

```{r, echo=TRUE}
# Initialize predictions as the mean value of y
preds <- mean(y)

# initialize the list of learners, error values, and data for plotting
learners <- rmse <- ggdata <- vector(mode='list', length = 100)

for(t in seq_along(learners)){
  
  df = tibble(x = x, y = y, residuals = y - preds)
  
  learners[[t]] <- lm(residuals ~ bs(x), data = df) #<<
  
  preds <- preds + 
    as.numeric(predict(learners[[t]], newdata = df)) * eta
...
Splines turn out to be a better weak learner than trees (in this case)
